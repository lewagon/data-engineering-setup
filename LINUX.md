...
- When prompted to `Select SSH configuration file to update`, pick the one in your home directory, under the `.ssh` folder, `/home/<username>/.ssh/config` basically. Usually VS Code will automatically pick the best option, so the default should work.
